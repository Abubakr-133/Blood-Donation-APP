...
import com.example.drblood.databinding.FragmentNotificationBinding;
...
public class MyRequestFragment extends Fragment {
    private myrequesttrackingadapter adapter;
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        FirebaseDatabase.getInstance().getReference("AcceptedRequests").addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot requestSnapshot) {
                ...
                for (DataSnapshot snapshot: requestSnapshot.getChildren())
                {
                    try {
                        ...
                        if (snapshot.child("requestorName").getValue(String.class).equals(FirebaseAuth.getInstance().getCurrentUser().getDisplayName())) {
...
//                                Requestordata request = new Requestordata(address, bloodGroup, requiredDate, patientName, patientphno, units, patientId);
                adapter = new myrequesttrackingadapter(requestList,requireContext());
